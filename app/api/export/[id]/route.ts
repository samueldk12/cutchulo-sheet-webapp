...
// GET /api/export/[id] - Full character sheet JSON export
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return new NextResponse(
      JSON.stringify({ version: 3, exportedAt: new Date().toISOString(), character: fullChar }, null, 2),
      {
        status: 200,
        headers: {
    ...
    console.error('Export character error:', e);
