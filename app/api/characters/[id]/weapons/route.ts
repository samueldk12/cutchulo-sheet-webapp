...
// POST /api/characters/[id]/weapons - Add a new empty weapon to a character
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await queryOne(
      'INSERT INTO weapons (character_id) VALUES ($1) RETURNING id',
      [characterId]
    );
    ...
    console.error('Create weapon error:', e);
